...
import scienceModel from "../models/scienceNewsModel.js";
class ScienceController {
  static createDoc = async (req, res) => {
    try {
      async function getscienceNews() {
        const res = await fetch(
          "https://newsapi.org/v2/everything?q=science&from=2022-10-12&sortBy=publishedAt&apiKey=fd1f88a4ca6d487c94e17b405c7fe7b5"
        );
        const response = await res.json();
        console.log(response.articles[0].author);
        for (let i = 0; i < response.articles.length; i++) {
          const newsdata = new scienceModel({
  ...
  static getScienceNews = async (req, res) => {
    try {
      try {
        const PAGE_SIZE = 10;
        const page = parseInt(req.query.page || "0");

        const total = await scienceModel.countDocuments();
        const result = await scienceModel
          .find()
          .limit(PAGE_SIZE)
          .skip(PAGE_SIZE * page);
        res.send({
